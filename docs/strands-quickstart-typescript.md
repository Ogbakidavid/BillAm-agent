...
First, ensure that you have Node.js 22+ and npm installed. See the npm documentation for installation instructions.
...
Learn more about the npm init command and its options.
...
- **Bedrock API keys**: Set the `AWS_BEARER_TOKEN_BEDROCK` environment variable
...
> **Note:** The `tool()` function also accepts plain JSON Schema objects instead of Zod. See Creating Custom Tools for details.

## Agent Loop

The agent operates through a cycle of reasoning and action:
...
**Reasoning (LLM)**: The language model analyzes the input and decides what to do.

**Input & Context**: The user's message and conversation history provide context for reasoning.

**Tool Selection**: The model determines which tools (if any) are needed for the task.

**Tool Execution**: Selected tools are executed and their results are collected.

**Response**: The model generates a final response based on tool results and reasoning.
...
To test our agent, we'll use tsx to run the file on Node.js:
...
- **OpenAI** - Access to OpenAI or OpenAI-compatible models
...
- [Examples](../examples) - Examples for many use cases
- [TypeScript SDK Repository](https://github.com/strands-ai/sdk-ts) - Explore the TypeScript SDK source code and contribute
- [Agent Loop](./concepts/agent-loop) - Learn how Strands agents work under the hood
- [State](./concepts/state) - Understand how agents maintain context and state across a conversation
- [Operating Agents in Production](./production) - Taking agents from development to production, operating them responsibly at scale
